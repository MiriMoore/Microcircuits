...
print('Mechanisms found: ', os.path.isfile('mod/arm64/special')) if RANK == 0 else None
...
if TESTING:
	...
	OUTPUTPATH = 'Circuit_output_fullsim/Healthy/'
...
if rec_LFP and not rec_DIPOLES:
    ...
    events = network.simulate(electrode=LFPelectrode, **simargs)
    SPIKES = events.get('times', None)
    OUTPUT = events.get('gids', None)
    DIPOLEMOMENT = None
...
if RANK==0:
	...
	output_file = os.path.join(OUTPUTPATH, 'SPIKES_Seed' + str(GLOBALSEED) + '.pkl')
	with open(output_file, 'wb') as f:
		pickle.dump(SPIKES, f)
	print(f"SPIKES saved successfully to {output_file}")
